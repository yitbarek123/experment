...
(define (va-em-add-reservation)
    ...
    ;     Person "company-2"...
     ...
     (let*
        ...
        ;reservation made
 (display "usr:")
        (if(equal? (Set ) user-location)
                va-false; (va-spt-add-atom em-atm 'reservation_made' #:timestamp current timestamp  user-latitude user-longitude)
                (display user-location); (va-spt-add-atom em-atm 'reservation_made' #:timestamp current timestamp)           
         )
 (newline)(display "restaurant:")
         ;reservation info
        (if(equal? (Set ) restaurant-location)
           va-false;(va-spt-add-atom em-atm 'reservation_info' #:timestamp  timestamp(date_time)  restaurant-latitude restaurant-longitude )
           (display restaurant-location); (va-spt-add-atom em-atm 'reservatio_info' #:timestamp current timestamp)         
         )
