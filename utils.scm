...
(define (va-get-generic-fact-1 predicate type)
    (if (va-fact-exist? predicate)
          (car (va-flat-node-list (cog-execute!
                 (GetLink
                   (TypedVariableLink
                    (VariableNode "$X")
                    (TypeNode (symbol->string type))
...
(define (get-va-user)
  (let ((query-result (cog-execute! (GetLink
                        (AndLink
                          (EvaluationLink
                            (PredicateNode "VA: this-va")
                            (ListLink (VariableNode "$V")))
                          (EvaluationLink
                            (PredicateNode "VA: is-va-user")
                            (ListLink (VariableNode "$P")
                            (VariableNode "$V"))))))))
      (gaar query-result))
)
...
; returns a ListLink containing two NumberNodes for latitude and longitude
; respectively.
(define (va-get-geolcation concept)
          (let ((query-result
                  (cog-execute!
                    (GetLink
                      (EvaluationLink
                        (PredicateNode "VA: geolocation")
                        (ListLink
                          concept
                          (VariableNode "$X")))))))
                (if (eq? query-result (ListLink))
                      query-result
